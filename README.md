...
1. ### The `systemstat` route doesn't work
    - Description: You can't make a request to get the system stats. That's it. You could do it with `systemstat: false`, but it's not included in this guide as it mostly won't impact the stack much
    - Fix: bruh, don't use it!
    - Severity: Low
    - Tracked by: Issue #3

2. ### Accidental pair when vehicle is inactive
